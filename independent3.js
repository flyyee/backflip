...
async function main() {
    ...
    async function man(itemName, itemURL) {
        ...
        do {
            ...
            if (scrape_res.sellorders.length > 0 && scrape_res.buyorders.length > 0) {
                ...
                for (let x = 0; x < scrape_res.sellorders.length; x++) {
                    if (scrape_res.sellorders[x].auto
                        && scrape_res.sellorders[x].online) {
                        sell_idx = x
                        break
                    }
                }
                for (let x = 0; x < scrape_res.buyorders.length; x++) {
                    if (scrape_res.buyorders[x].auto
                        && scrape_res.buyorders[x].online
                        && !scrape_res.buyorders[x].painted) {
                        buy_idx = x
                        break
                    }
                }
            ...
            const bm = require("./bmon")
            let bmonMan = new MyEmitter()
            await bm.check(bmonMan, scrape_res.buyorders[buy_idx].list_price, m_sudts, scrape_res.buyorders[buy_idx].suid, 15000)
                .then(res => {
                    return trade.trade(m_sudts, my_items, their_items, their_tradeURL, 100)
                }) // TODO: store their items/metal in trade setup too
                .then(sentOffer_id => {
                    console.log(my_bank)
                    ...
                    bm.start(bmonMan, scrape_res.buyorders[buy_idx].list_price, m_sudts, scrape_res.buyorders[buy_idx].suid)
                    // const buyerMonDelay = 15000
                    ...
                    //     await buyer_bank.refreshInvR(3, m_sudts.manager, scrape_res.buyorders[buy_idx].suid, null)
                    //         .catch((err) => {
                    //             console.log(err) // error with getting inventory
                    //             const a = buyer_bank, b = m_sudts.manager, c = scrape_res.buyorders[buy_idx].suid, d = null
                    ...
                    //     try {
                    //         metal.allocateMetal(their_bank, my_sellprice, tradecount_idx)
                    //     } catch (err) {
                    //         if (err == "Insufficient currency: 042x11") {
                    ...
                    return trade.tradeResponse(sentOffer_id, 1, bmonMan)
                })
                ...
                .then(res => {
                    ...
                    return trade.trade(m_sudts, res.my_items, res.their_items, res.their_tradeURL, 102) // TODO: store their items/metal in trade setup too
                    // .catch(err => {
                ...
                .catch(err => {
                    ...
                    if (err[0] == "trade") {
                        let return_value = err[4]
                        if (id == 100 || id == 101) {
                            console.log(err) // some error with sending the trade
                            blacklist.push1(scrape_res.sellorders[sell_idx].data["data-listing_account_id"])
                            gotoman = true
